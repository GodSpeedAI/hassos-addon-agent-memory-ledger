...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    if len(description) > 1024:
        transcript["rewrite_attempts"] = []
        max_retries = 3
        for attempt in range(1, max_retries + 1):
            shorten_prompt = (
                f"Your description is {len(description)} characters, which exceeds the hard 1024 character limit. "
                f"Rewrite it under 1024 characters while preserving trigger words and intent coverage. "
                f"Attempt {attempt}/{max_retries}. Respond only with <new_description>...</new_description>."
            )
            ...
            attempt_record = {
                ...
                "thinking": shorten_thinking,
                "response": shorten_text,
                "description": shortened,
                "char_count": len(shortened),
            }
            transcript["rewrite_attempts"].append(attempt_record)
            description = shortened
            if len(shortened) <= 1024:
                break
        if len(description) > 1024:
            raise ValueError(
                f"Failed to shorten description under 1024 characters after {max_retries} attempts"
            )
...
def main():
    ...
    parser.add_argument(
        "--log-dir", default=None, help="Directory to log transcript JSON"
    )
    parser.add_argument(
        "--iteration", default=None, help="Iteration number for logging"
    )
