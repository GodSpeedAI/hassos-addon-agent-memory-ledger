        ...
        for config_dir in sorted(eval_dir.iterdir()):
            ...
            for run_dir in sorted(config_dir.glob("run-*")):
                try:
                    ...
                    print(
                        f"Warning: skipping malformed run directory name: {run_dir.name}"
                    )
                    continue
                ...
                for exp in raw_expectations:
                    if "text" in exp and "passed" in exp and "evidence" in exp:
                        filtered_expectations.append(exp)
                    else:
                        print(
                            f"Warning: expectation in {grading_file} missing required fields (text, passed, evidence): {exp}"
                        )
                result["expectations"] = filtered_expectations
...
def generate_benchmark(
    ...
    # Always use runs_by_config as a dict to maintain consistent type
    runs_per_configuration = runs_by_config if runs_by_config else {}
...
def generate_markdown(benchmark: dict) -> str:
    ...
    if isinstance(delta_pass_rate, (int, float)):
        delta_pass_rate_str = f"{delta_pass_rate * 100:+.0f}%"
    else:
        delta_pass_rate_str = "—"
    lines.append(
        f"| Pass Rate | {a_pr.get('mean', 0) * 100:.0f}% ± {a_pr.get('stddev', 0) * 100:.0f}% | {b_pr.get('mean', 0) * 100:.0f}% ± {b_pr.get('stddev', 0) * 100:.0f}% | {delta_pass_rate_str} |"
    )
